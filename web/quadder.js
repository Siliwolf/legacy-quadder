...
function acquireVertexArrayObjectExtension(e) {
...
function acquireInstancedArraysExtension(e) {
...
function acquireDisjointTimerQueryExtension(e) {
      ...
      e[t].version == null
        ...
        : ((n = e[t].name + "_crate_version"),
          wasm_exports[n] == void 0
            ...
            : ((s = u32_to_semver(wasm_exports[n]())),
              e[t].version != s &&
                console.error(
                  "Plugin " +
                    e[t].name +
    ...
    typeof WebAssembly.compileStreaming == "function"
      ? WebAssembly.compileStreaming(t)
          ...
          .then((e) => {
            ...
            var t = u32_to_semver(wasm_exports.crate_version());
            version != t &&
              console.error(
...
function audio_add_buffer(e, t) {
  let s = wasm_memory.buffer.slice(e, e + t),
    n = sound_key_next;
  return (
    (sound_key_next += 1),
...
function audio_source_is_loaded(e) {
  return sounds.has(e) && sounds.get(e) != void 0;
}
function recycle_playback() {
  let e = playbacks.find((e) => e.sound_key === 0);
  return (
    e != null
      ? (e.source = audio_context.createBufferSource())
      : ((e = {
          sound_key: 0,
          playback_key: 0,
...
function audio_play_buffer(e, t, n) {
  let o = playback_key_next++,
    s = recycle_playback();
  (s.sound_key = e),
    (s.playback_key = o),
    s.source.connect(s.gain_node),
    s.gain_node.connect(audio_context.destination),
    (s.gain_node.gain.value = t),
    (s.source.loop = n),
    (s.ended = function () {
      stop(s);
    }),
    s.source.addEventListener("ended", s.ended);
  try {
    (s.source.buffer = sounds.get(e)), s.source.start(0);
  } catch (e) {
    console.error("Error starting sound", e);
  }
  return o;
}
function audio_source_set_volume(e, t) {
  playbacks.forEach((n) => {
    n.sound_key === e && (n.gain_node.gain.value = t);
  });
}
function audio_source_stop(e) {
  playbacks.forEach((t) => {
    t.sound_key === e && stop(t);
  });
}
function audio_source_delete(e) {
  audio_source_stop(e), sounds.delete(e);
}
function audio_playback_stop(e) {
  let t = playbacks.find((t) => t.playback_key === e);
  t != null && stop(t);
}
function audio_playback_set_volume(e, t) {
  let n = playbacks.find((t) => t.playback_key === e);
  n != null && (n.gain_node.gain.value = t);
}
function register_plugin(e) {
  (e.env.audio_init = audio_init),
    (e.env.audio_add_buffer = audio_add_buffer),
    (e.env.audio_play_buffer = audio_play_buffer),
    (e.env.audio_source_is_loaded = audio_source_is_loaded),
    (e.env.audio_source_set_volume = audio_source_set_volume),
    (e.env.audio_source_stop = audio_source_stop),
    (e.env.audio_source_delete = audio_source_delete),
    (e.env.audio_playback_stop = audio_playback_stop),
    (e.env.audio_playback_set_volume = audio_playback_set_volume);
}
miniquad_add_plugin({
  register_plugin,
  version: "0.1.0",
...
(ctx = null),
  ...
  (register_plugin = function (e) {
    (e.env.js_create_string = function (e, t) {
      var n = UTF8ToString(e, t);
      return js_object(n);
    }),
      (e.env.js_create_buffer = function (e, t) {
        var n = new Uint8Array(wasm_memory.buffer, e, t),
          s = new Uint8Array(new ArrayBuffer(n.byteLength));
        return s.set(new Uint8Array(n)), js_object(s);
      }),
      (e.env.js_create_object = function () {
        var e = {};
        return js_object(e);
      }),
      (e.env.js_set_field_f32 = function (e, t, n, s) {
        var o = UTF8ToString(t, n);
        js_objects[e][o] = s;
      }),
      (e.env.js_set_field_u32 = function (e, t, n, s) {
        var o = UTF8ToString(t, n);
        js_objects[e][o] = s;
      }),
      (e.env.js_set_field_string = function (e, t, n, s, o) {
        var i = UTF8ToString(t, n),
          a = UTF8ToString(s, o);
        js_objects[e][i] = a;
      }),
      (e.env.js_unwrap_to_str = function (e, t, n) {
        for (
          var i = js_objects[e],
            o = toUTF8Array(i),
            a = o.length,
            r = new Uint8Array(wasm_memory.buffer, t, n),
            s = 0;
          s < a;
          s++
        )
      ...
      (e.env.js_unwrap_to_buf = function (e, t, n) {
        for (
          var o = js_objects[e],
            i = o.length,
            a = new Uint8Array(wasm_memory.buffer, t, n),
            s = 0;
          s < i;
          s++
        )
      ...
      (e.env.js_string_length = function (e) {
        var t = js_objects[e];
        return toUTF8Array(t).length;
      }),
      (e.env.js_buf_length = function (e) {
        var t = js_objects[e];
        return t.length;
      }),
      (e.env.js_free_object = function (e) {
        delete js_objects[e];
      }),
      (e.env.js_have_field = function (e, t, n) {
        var s = UTF8ToString(t, n);
        return js_objects[e][s] !== void 0;
      }),
      (e.env.js_field_f32 = function (e, t, n) {
        var s = UTF8ToString(t, n);
        return js_objects[e][s];
      }),
      (e.env.js_field_u32 = function (e, t, n) {
        var s = UTF8ToString(t, n);
        return js_objects[e][s];
      }),
      (e.env.js_field = function (e, t, n) {
        var s = UTF8ToString(t, n),
          o = js_objects[e][s];
        return js_object(o);
      }),
      (e.env.js_field_num = function (e, t, n) {
        var s = UTF8ToString(t, n);
        return js_objects[e][s];
      });
  }),
...
function js_object(e) {
  if (e == null) return -2;
  if (e === null) return -1;
  var t = unique_js_id;
  return (js_objects[t] = e), (unique_js_id += 1), t;
}
function consume_js_object(e) {
  var t = js_objects[e];
  return delete js_objects[e], t;
}
function get_js_object(e) {
  return js_objects[e];
}
function on_init() {}
(register_plugin = function (e) {
  (e.env.ws_connect = ws_connect),
    (e.env.ws_is_connected = ws_is_connected),
    (e.env.ws_send = ws_send),
    (e.env.ws_try_recv = ws_try_recv),
    (e.env.http_make_request = http_make_request),
    (e.env.http_try_recv = http_try_recv);
}),
  miniquad_add_plugin({
    register_plugin,
    on_init,
...
function ws_connect(e) {
  (quad_socket = new WebSocket(consume_js_object(e))),
    (quad_socket.binaryType = "arraybuffer"),
    (quad_socket.onopen = function () {
      connected = 1;
    }),
    (quad_socket.onmessage = function (e) {
      if (typeof e.data == "string")
        received_buffer.push({ text: 1, data: e.data });
      else {
        var t = new Uint8Array(e.data);
        received_buffer.push({ text: 0, data: t });
      }
    });
}
function ws_send(e) {
  var t = consume_js_object(e);
  t.buffer != void 0 ? quad_socket.send(t.buffer) : quad_socket.send(t);
}
function ws_try_recv() {
  return received_buffer.length != 0 ? js_object(received_buffer.shift()) : -1;
}
(uid = 0), (ongoing_requests = {});
function http_try_recv(e) {
  if (ongoing_requests[e] != void 0 && ongoing_requests[e] != null) {
    var t = ongoing_requests[e];
    return (ongoing_requests[e] = null), js_object(t);
  }
  return -1;
}
function http_make_request(e, t, n, s) {
  var o,
    i,
    a,
    c,
  ...
  (uid += 1),
    ...
    e == 2 && (i = "GET"),
    e == 3 && (i = "DELETE"),
    (c = consume_js_object(t)),
    (l = consume_js_object(n)),
    (a = consume_js_object(s)),
    (o = new XMLHttpRequest()),
    o.open(i, c, !0),
    (o.responseType = "arraybuffer");
  for (const e in a) o.setRequestHeader(e, a[e]);
  return (
    (o.onload = function () {
      if (this.status == 200) {
        var t = new Uint8Array(this.response);
        ongoing_requests[r] = t;
      }
    }),
