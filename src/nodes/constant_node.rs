...
pub fn draw_constant_node(
    ...
    if node.selected && state.clone().cur_const_type.is_some() {
        node.output_connectors[0].connector_type = state.cur_const_type.clone().unwrap();
        node.name = state.cur_const_data.clone().unwrap();
    }
        ...
        && (lmc.y > y - (height / 2.0) && lmc.y < y - (height / 2.0) + height)
    {
        ...
        {
            node.selected = true;
            state.cur_const_type = Some(node.output_connectors[0].connector_type.clone());
            state.cur_const_data = Some(node.name.clone());
        }
    }
