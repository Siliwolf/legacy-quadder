...
pub fn draw_variable_node(
    ...
    // Name Culling
    let mut node_name = match state.cur_var_data.clone().is_some() {
        true => {
            node.name = state.cur_var_data.clone().unwrap();
            state.cur_var_data.clone().unwrap()
        },
        false => {
            node.name.clone()
        },
    ...
    if node.selected && state.clone().cur_var_type.is_some() {
        node.output_connectors[0].connector_type = state.cur_var_type.clone().unwrap();
        node.name = state.cur_var_data.clone().unwrap();
    }
        ...
        && (lmc.y > y - (height / 2.0) && lmc.y < y - (height / 2.0) + height)
    {
        ...
        {
            node.selected = true;
            state.cur_var_type = Some(node.output_connectors[0].connector_type.clone());
            state.cur_var_data = Some(node.name.clone());
        }
    }
